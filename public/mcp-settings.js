...
async function removeMcpServer(serverName) {
    const confirmed = await window.customConfirm({
        title: 'Remove Server',
        message: `Are you sure you want to remove the server "${serverName}"?`,
        confirmText: 'Remove',
        type: 'danger'
    });
...
//=========================================
let mcpFormMode = 'simple';
let mcpSelectedTemplate = null;

function openMcpModal() {
    const inlineForm = document.getElementById('mcp-add-server-inline');
    inlineForm.style.display = 'block';
    
    // Reset to simple mode
    setMcpFormMode('simple');
    
    // Clear form
    document.getElementById('mcp-inline-type').value = '';
    document.getElementById('mcp-inline-name').value = '';
    document.getElementById('mcp-inline-command').value = 'npx';
    document.getElementById('mcp-inline-args').value = '';
    document.getElementById('mcp-inline-env').value = '';
    document.getElementById('mcp-simple-config').innerHTML = '';
    document.getElementById('mcp-simple-config').style.display = 'none';
    mcpSelectedTemplate = null;
    
    // Focus first input
    setTimeout(() => {
        document.getElementById('mcp-inline-type').focus();
    }, 100);
}

function setMcpFormMode(mode) {
    mcpFormMode = mode;
    const simpleBtn = document.querySelector('.mcp-mode-btn[data-mode="simple"]');
    const advancedBtn = document.querySelector('.mcp-mode-btn[data-mode="advanced"]');
    const simpleField = document.querySelector('.mcp-simple-field');
    const simpleConfig = document.getElementById('mcp-simple-config');
    const advancedFields = document.getElementById('mcp-advanced-fields');
    ...
    if (mode === 'simple') {
        ...
        // Show simple config if a template is selected
        if (mcpSelectedTemplate && mcpSelectedTemplate !== 'custom') {
...
function onMcpServerTypeChange() {
    const typeSelect = document.getElementById('mcp-inline-type');
    const nameInput = document.getElementById('mcp-inline-name');
    const commandInput = document.getElementById('mcp-inline-command');
    const argsTextarea = document.getElementById('mcp-inline-args');
    const envTextarea = document.getElementById('mcp-inline-env');
    const simpleConfig = document.getElementById('mcp-simple-config');
    
    const selectedType = typeSelect.value;
    mcpSelectedTemplate = selectedType;
    
    if (!selectedType) {
        simpleConfig.style.display = 'none';
        simpleConfig.innerHTML = '';
        return;
    }
    ...
    if (selectedType === 'custom') {
        ...
        setMcpFormMode('advanced');
        return;
    }
    
    const template = mcpState.templates[selectedType];
    if (!template) {
        simpleConfig.style.display = 'none';
        return;
    }
    ...
    // Show simple config fields for required configuration
    if (template.configFields && template.configFields.length > 0) {
        const configHtml = template.configFields.map(field => {
            const inputType = field.type === 'password' ? 'password' : 'text';
            const requiredMark = field.required ? '<span class="mcp-required">*</span>' : '';
            return `
                <div class="mcp-form-group">
                    <label>${escapeHtml(field.label)} ${requiredMark}</label>
                    <input type="${inputType}" 
                           id="mcp-config-${escapeHtml(field.name)}" 
                           class="mcp-input mcp-config-field" 
                           data-field="${escapeHtml(field.name)}"
                           data-is-arg="${field.isArg || false}"
                           data-arg-index="${field.argIndex !== undefined ? field.argIndex : -1}"
        ...
        simpleConfig.innerHTML = configHtml;
        simpleConfig.style.display = 'block';
    } else if (template.description) {
        simpleConfig.innerHTML = `
            <div class="mcp-template-description">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <circle cx="12" cy="12" r="10"></circle>
...
async function addMcpServerInline() {
    const name = document.getElementById('mcp-inline-name').value.trim();
    let command = document.getElementById('mcp-inline-command').value.trim();
    let argsText = document.getElementById('mcp-inline-args').value;
    let envText = document.getElementById('mcp-inline-env').value;
    
    // In simple mode, apply config field values
    if (mcpFormMode === 'simple' && mcpSelectedTemplate && mcpSelectedTemplate !== 'custom') {
        const template = mcpState.templates[mcpSelectedTemplate];
        if (template) {
            command = template.command;
            const argsArray = [...template.args];
            const envObj = { ...template.env };
            
            // Process config fields
            const configFields = document.querySelectorAll('.mcp-config-field');
            configFields.forEach(field => {
                const fieldName = field.dataset.field;
                const isArg = field.dataset.isArg === 'true';
                const argIndex = parseInt(field.dataset.argIndex, 10);
                const value = field.value.trim();
                
                if (isArg && argIndex >= 0) {
                    argsArray[argIndex] = value;
                } else {
                    envObj[fieldName] = value;
                }
            });
    ...
    const addBtn = document.getElementById('mcp-inline-add-btn');
    ...
    try {
        ...
        if (result.success) {
            ...
            showMcpNotification(result.error || 'MCP connection limit reached. Upgrade your plan for more connections.', 'error');
        } else {
...
function showTemplateConfigModal(template) {
    ...
    // Set the template type in dropdown
    if (typeSelect) {
        typeSelect.value = template.name;
    }
    mcpSelectedTemplate = template.name;
    ...
    // Trigger the type change to populate simple config
    onMcpServerTypeChange();
    
    inlineForm.style.display = 'block';
    
    // Switch to servers tab to show the form
    const serversTab = document.querySelector('.mcp-tab[data-tab="servers"]');
    if (serversTab) {
        serversTab.click();
    }
}
...
window.closeMcpInlineForm = closeMcpInlineForm;
window.addMcpServer = addMcpServer;
window.addMcpServerInline = addMcpServerInline;
...
window.setMcpFormMode = setMcpFormMode;
window.onMcpServerTypeChange = onMcpServerTypeChange;
