...
class SubscriptionManager {
  ...
  canSendMessage() {
    ...
    if (messagesUsed >= tierInfo.messagesPerDay) {
      return {
        allowed: false,
        reason: `Daily message limit reached (${tierInfo.messagesPerDay} messages). Upgrade your plan for more.`,
        remaining: 0
      };
    }
  ...
  canAddMCPConnection(currentConnectionCount = 0) {
    ...
    if (tierInfo.mcpConnections === 0) {
      return {
        allowed: false,
        reason: 'MCP connections not available in Free tier. Upgrade to Super-Red or Ultra-Red.'
      };
    }
    ...
    if (currentConnectionCount >= tierInfo.mcpConnections) {
      return {
        allowed: false,
        reason: `MCP connection limit reached (${tierInfo.mcpConnections} connections). Remove existing connections or upgrade your plan.`,
        remaining: 0
      };
    }
  ...
  canUseTranscription() {
    ...
    if (tierInfo.transcriptionHours === 0) {
      return {
        allowed: false,
        reason: 'Transcription not available in Free tier. Upgrade to Super-Red or Ultra-Red.'
      };
    }
    ...
    if (minutesUsed >= totalMinutesAllowed) {
      return {
        allowed: false,
        reason: `Monthly transcription limit reached (${tierInfo.transcriptionHours} hours). Upgrade your plan for more.`,
        remaining: 0
      };
    }
