...
const { getUpdater } = require('./auto-updater');
...
async function createAuthWindow() {
  ...
  // Create a simple waiting window
  ...
  // No longer auto-opening browser - user will click button to start auth
  console.log('🔐 Auth window ready, waiting for user to click sign in');
...
function createWindow() {
  ...
  // Set the main window reference in protocol handler
...
async function processScreenshot(imageData, quality = 'medium') {
  try {
    const qualitySettings = {
      low: { scaleFactor: 0.15 },
      medium: { scaleFactor: 0.2 },
      high: { scaleFactor: 0.25 }
    };

    const settings = qualitySettings[quality] || qualitySettings.medium;

    // Use Electron's nativeImage for processing (no native dependencies)
    let image = nativeImage.createFromBuffer(Buffer.from(imageData));
    
    // Resize the image
    const originalSize = image.getSize();
    const newHeight = Math.round(originalSize.height * settings.scaleFactor);
    const newWidth = Math.round(originalSize.width * settings.scaleFactor);
...
async function handleScreenCaptureShortcut() {
  if (!mainWindow) {
    console.log('⚠️ Screen capture shortcut triggered but mainWindow is not defined');
    return;
  }
  try {
...
ipcMain.handle('check-for-updates', async () => {
  try {
    ...
    console.error('[Main] Check for updates failed:', error);
...
ipcMain.handle('download-update', async () => {
  try {
    ...
    console.error('[Main] Download update failed:', error);
    return { error: error.message };
  }
});
...
ipcMain.handle('install-update', async () => {
  try {
    ...
    console.error('[Main] Install update failed:', error);
    return { error: error.message };
  }
});
...
ipcMain.handle('get-update-status', async () => {
  try {
    ...
    console.error('[Main] Get update status failed:', error);
    return { error: error.message };
  }
});
...
// Set up authentication handlers for deep linking
protocolHandler.setAuthTokenHandler(async (token) => {
  console.log('🎉 Auth token received from protocol');
  
  try {
    const userData = await authManager.fetchUserData(token);
    
    if (userData.success) {
      authManager.saveAuth(token, userData.user);
      ...
      if (authWindow && !authWindow.isDestroyed()) {
        ...
        if (mainWindow) {
          mainWindow.webContents.send('auth-success', userData.user);
        } else {
          createWindow();
        }
      }
    ...
    console.error('❌ Error in auth token handler:', error);
    if (authWindow && !authWindow.isDestroyed()) {
...
protocolHandler.setAuthCodeHandler(async (code) => {
  console.log('🎉 Auth code received from protocol');
  ...
  try {
    ...
    const userData = await authManager.fetchUserData(access_token);
    
    if (userData.success) {
      authManager.saveAuth(access_token, userData.user);
      saveAuthData(userData.user);
    ...
    console.error('❌ Error in auth code handler:', error);
    if (authWindow && !authWindow.isDestroyed()) {
      authWindow.webContents.send('auth-error', {
        error: error.message || 'Authentication failed'
      });
...
// Set up HTTP callback handler (fallback for when protocol doesn't work in dev)
authManager.setAuthSuccessCallback(async (token) => {
  console.log('🎉 Auth token received via HTTP callback!');
  ...
  try {
    ...
    if (userData.success) {
      ...
      if (authWindow && !authWindow.isDestroyed()) {
        // Send success event to auth window first
        authWindow.webContents.send('auth-success', userData.user);
        
        // Then load success page after brief delay
        setTimeout(() => {
          if (authWindow && !authWindow.isDestroyed()) {
            authWindow.loadFile(path.join(__dirname, '../public/auth-success.html'));
          }
    ...
    console.error('❌ Error in HTTP auth callback:', error);
    if (authWindow && !authWindow.isDestroyed()) {
      authWindow.webContents.send('auth-error', {
        error: error.message || 'Authentication failed'
      });
  ...
  setTimeout(async () => {
    try {
      ...
      if (result.updateAvailable) {
        console.log('[Main] Update available, prompting user...');
        await updater.promptUserForUpdate();
      }
    } catch (error) {
      console.error('[Main] Update check failed:', error.message);
    }
  }, 5000);
...
ipcMain.handle('auth-success', async (event, user) => {
  try {
    ...
    // Clear protocol handler reference to avoid focusing destroyed window
    protocolHandler.setMainWindow(null);
...
ipcMain.handle('auth-get-current-user', async () => {
  ...
  console.log('📋 Getting current user:', user?.email || 'Not authenticated');
  return { 
    success: !!user,
    user: user,
...
// Start sign-in flow when user clicks the sign-in button
ipcMain.handle('start-signin-flow', async () => {
  console.log('🔐 User initiated sign-in flow');
