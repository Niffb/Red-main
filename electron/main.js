...
// Create authentication window - shows sign-in screen first
function createAuthWindow() {
  ...
  // Load the sign-in required screen (user must click to proceed)
  authWindow.loadFile(path.join(__dirname, '../public/signin-required.html'));
...
// Start the OAuth flow when user clicks sign-in
async function startOAuthFlow() {
  if (!authWindow || authWindow.isDestroyed()) {
    console.error('❌ Auth window not available');
    return { success: false, error: 'Auth window not available' };
  }
  ...
  try {
    ...
    // Also set user in subscriptionManager so settings window can access it
    subscriptionManager.setCurrentUser({
      email: user.email,
    ...
    console.log('✅ User set in subscriptionManager');
...
function createWindow() {
  ...
  // Restore user to subscriptionManager from authManager for settings access
  ...
  if (authUser) {
    ...
    subscriptionManager.setCurrentUser({
      ...
      fullName: authUser.full_name || authUser.fullName || authUser.name,
      name: authUser.name,
      avatar: authUser.avatar,
      plan: authUser.plan || 'Free',
      subscription: authUser.subscription || {
        tier: authUser.plan || 'Free',
        status: 'active',
        limits: authUser.subscription?.limits || {}
      },
      usage: authUser.usage || {}
    });
    console.log('✅ User restored to subscriptionManager:', authUser.email);
  }
...
function registerShortcut() {
  ...
  if (!isFirstRun) {
    setTimeout(() => {
      if (!isVisible && mainWindow && !mainWindow.isDestroyed() && isAuthenticated) {
...
async function callOpenRouterAPIWithScreen({ message, screenshot, conversationHistory = [] }) {
  ...
  try {
    ...
    messages.push({
      role: 'system',
      content: 'You are RED AI, the intelligent assistant powering RED — a multi-modal AI desktop application. The user has provided a screenshot of their screen as visual context. Use the screenshot only when relevant to answering their question. Focus on answering directly. Only reference what\'s on screen if it\'s directly relevant. If the question doesn\'t require visual context, answer normally without mentioning the screenshot.'
    });
...
ipcMain.handle('ai-text', async (event, { text, imageBase64 = null, mimeType = 'image/png', conversationHistory = [] }) => {
  try {
    ...
    const messageCheck = subscriptionManager.canSendMessage();
    if (!messageCheck.allowed) {
      console.log('⚠️ Message limit reached:', messageCheck.reason);
      return { 
        success: false, 
        error: messageCheck.reason,
        rateLimited: true,
        remaining: messageCheck.remaining || 0
      };
    }
    ...
    const body = {
      ...
      systemInstruction: functionDeclarations.length > 0 ? {
        parts: [{
          text: `You are RED AI, the intelligent assistant powering RED — a multi-modal AI desktop application for productivity automation.

═══════════════════════════════════════════════════════════════════════════
CORE IDENTITY
═══════════════════════════════════════════════════════════════════════════

You are an action-oriented AI that executes tasks through the Model Context Protocol (MCP). Unlike typical assistants that only provide information, you directly interact with the user's digital world.

FUNDAMENTAL PRINCIPLES:
• Execute requests immediately using available tools — don't suggest manual steps
• Chain multiple tools together for complex tasks
• Ask for clarification only when essential parameters are missing
• Report results concisely and confirm completion

═══════════════════════════════════════════════════════════════════════════
RED APP CAPABILITIES YOU CAN LEVERAGE
═══════════════════════════════════════════════════════════════════════════

WORKFLOW AUTOMATION:
• Create automated workflows triggered by keywords, schedules, or manual activation
• Chain AI prompts, notifications, clipboard operations, and HTTP requests
• Schedule tasks: daily, weekly, monthly, or at specific times
• Help users build workflows from natural language descriptions

TRANSCRIPTION:
• Real-time speech-to-text transcription via Deepgram integration
• Generate workflow suggestions from transcribed conversations
• Save and analyse transcription history

SCREEN ANALYSIS:
• Analyse screenshots and visual context when provided
• Help users understand what's on their screen
• Extract text and information from images

═══════════════════════════════════════════════════════════════════════════
MCP TOOLS — YOUR HANDS IN THE DIGITAL WORLD
═══════════════════════════════════════════════════════════════════════════

FILESYSTEM:
• Read, write, create, and delete files
• List directories and search for files
• Use absolute paths appropriate to the user's OS

NOTION:
• Create pages and databases
• Append content to existing pages
• Search across the user's Notion workspace
Keywords: "save to notion", "create page", "add note", "search my notes"

GITHUB:
• Create and manage issues
• Search repositories and code
• Check commit history and pull requests
Keywords: "create issue", "check repo", "search code"

SLACK:
• Send messages to channels or users
• Search conversations
• Read channel history
Keywords: "send to slack", "message team", "check channel"

GOOGLE DRIVE:
• List and search files
• Read document contents
• Organise and manage files
Keywords: "find in drive", "search documents"

DATABASE:
• Execute SQL queries
• Inspect table structures
• Retrieve and analyse data
Keywords: "query database", "check table", "run SQL"

OTHER MCP SERVERS:
• Automatically discover and use any connected MCP server
...
1. INTERPRET: What does the user want to achieve?
2. IDENTIFY: Which tools can accomplish this?
3. EXECUTE: Run tools confidently, chain if needed
4. REPORT: Confirm what was done with specifics

WHEN REQUESTS ARE VAGUE:
• Use conversation context
• Make reasonable assumptions
• Only ask for essential missing info (file paths, page IDs, repo names)
...
✓ Confident: "Done — I've added that to your Notion page"
✓ Concise: Report results, not intentions
✓ Proactive: Offer relevant follow-up actions
✓ Honest: Explain failures clearly with alternatives

✗ Don't say "You could do X" when you can do it
✗ Don't explain what you're about to do — just do it
✗ Don't say "I cannot" if you have tools that can help

═══════════════════════════════════════════════════════════════════════════
You are not just an AI that knows things. You are RED AI — an AI that DOES things.
═══════════════════════════════════════════════════════════════════════════`
        }]
      } : undefined,
...
═══════════════════════════════════════════════════════════════════════════`
    ...
    // Increment message usage even for max iterations
    await subscriptionManager.incrementMessageUsage();
...
ipcMain.handle('mcp-add-server', async (event, { serverName, command, args, env }) => {
  try {
    // Check rate limiting for MCP connections
    const currentCount = mcpManager.servers ? mcpManager.servers.size : 0;
    const mcpCheck = subscriptionManager.canAddMCPConnection(currentCount);
    if (!mcpCheck.allowed) {
      console.log('⚠️ MCP connection limit reached:', mcpCheck.reason);
      return { 
        success: false, 
        error: mcpCheck.reason,
        rateLimited: true,
        remaining: mcpCheck.remaining || 0
      };
    }
...
// Register focus callback to bring existing window to front when protocol URL is received
protocolHandler.onFocusRequest(() => {
  console.log('🔍 Protocol handler requesting window focus...');

  // Focus auth window if it exists (during sign-in flow)
  if (authWindow && !authWindow.isDestroyed()) {
    ...
    console.log('✅ Focused auth window');
    return;
  }
...
app.whenReady().then(async () => {
  ...
  // Set fallback timer to show window (only if mainWindow exists and not in auth flow)
  setTimeout(() => {
    if (!isVisible && mainWindow && !mainWindow.isDestroyed() && isAuthenticated) {
      console.log('🔧 Fallback: Showing window after 3 seconds');
      toggleWindow();
  ...
  switch (step.type) {
    ...
    case 'notification':
      ...
      try {
        ...
        const title = step.config?.title || 'RED AI Workflow';
...
// Local transcription storage path
const transcriptionsFilePath = path.join(app.getPath('userData'), 'transcriptions.json');

// Load local transcriptions
function loadLocalTranscriptions() {
  try {
    ...
    console.error('Error loading local transcriptions:', error);
  }
  return [];
}

// Save local transcriptions
function saveLocalTranscriptions(transcriptions) {
  try {
    fs.writeFileSync(transcriptionsFilePath, JSON.stringify(transcriptions, null, 2));
  } catch (error) {
    console.error('Error saving local transcriptions:', error);
  }
}

// Save transcription (local JSON storage)
ipcMain.handle('transcription-save', async (event, transcriptionData) => {
  try {
    ...
    const transcription = {
      ...
      preview: transcriptionData.preview || transcriptionData.text?.substring(0, 100) + '...',
      timestamp: transcriptionData.timestamp || new Date().toISOString(),
      createdAt: new Date().toISOString()
    };

    // Load existing transcriptions and add new one
    const transcriptions = loadLocalTranscriptions();
    ...
    saveLocalTranscriptions(transcriptions);

    console.log('✅ Transcription saved locally:', transcription.id);
...
// Get user's transcription library (local JSON storage)
...
// Delete transcription (local JSON storage)
...
ipcMain.handle('transcription-start', async (event) => {
  try {
    ...
    const transcriptionCheck = subscriptionManager.canUseTranscription();
    if (!transcriptionCheck.allowed) {
      console.log('⚠️ Transcription limit reached:', transcriptionCheck.reason);
      return { 
        success: false, 
        error: transcriptionCheck.reason,
        rateLimited: true,
        remaining: transcriptionCheck.remaining || 0
      };
    }
...
ipcMain.handle('transcription-stop', async (event) => {
  try {
    ...
    // Update transcription usage if we have duration data
    if (result && result.duration) {
      const minutesUsed = Math.ceil(result.duration / 60);
      await subscriptionManager.updateTranscriptionUsage(minutesUsed);
      console.log(`📊 Transcription usage updated: ${minutesUsed} minute(s)`);
    }
...
// Start OAuth flow when user clicks sign-in button
...
ipcMain.handle('auth-refresh-user-data', async () => {
  ...
  // Also update subscriptionManager with refreshed data
  if (result.success && result.user) {
    ...
    console.log('✅ User data refreshed in subscriptionManager');
  }
