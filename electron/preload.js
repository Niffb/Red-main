...
window.electronAPI = {
  ...
  startSignInFlow: () => ipcRenderer.invoke('start-signin-flow'),
  startGoogleAuth: () => ipcRenderer.invoke('start-google-auth'),
  authenticateUser: (endpoint, data) => ipcRenderer.invoke('authenticate-user', { endpoint, data }),
  
  // Generic event listener (used for auth-error, auth-success, etc.)
  on: (channel, callback) => {
