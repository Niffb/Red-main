...
class WorkflowExecutor {
  constructor(mcpManager) {
    ...
    // Use userData directory for writable storage (works in packaged apps)
    const userDataPath = app.getPath('userData');
    const dataDir = path.join(userDataPath, 'data');
    this.workflowsPath = path.join(dataDir, 'workflows.json');
    ...
    // Copy default workflows if none exist
  ...
  /**
   * Initialize with default workflows if user data is empty
   */
  initializeDefaultWorkflows() {
    if (!fs.existsSync(this.workflowsPath)) {
      // Try to copy from bundled data
      const bundledPath = app.isPackaged
        ? path.join(process.resourcesPath, 'data', 'workflows.json')
        : path.join(__dirname, '../data/workflows.json');
      
      if (fs.existsSync(bundledPath)) {
        try {
          fs.copyFileSync(bundledPath, this.workflowsPath);
          console.log('📋 Copied default workflows to user data');
        } catch (err) {
          console.warn('⚠️ Could not copy default workflows:', err.message);
        }
      }
    }
  }
